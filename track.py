...
class HMR2023TextureSampler(HMR2Predictor):
    ...
    def forward(self, x):
        ...
        def unproject_uvmap_to_mesh(bmap, fmap, verts, faces):
            ...
            fmap_flat = fmap[valid_mask]
            bmap_flat = bmap[valid_mask,:]

            face_vids = faces[fmap_flat, :]
            face_verts = verts[:, face_vids, :]

            map_verts = torch.einsum('bnij,ni->bnj', face_verts, bmap_flat)
        ...
        map_verts, valid_mask = unproject_uvmap_to_mesh(self.tex_bmap, self.tex_fmap, pred_verts, face_tensor)
        ...
        rend_depth_at_proj = torch.nn.functional.grid_sample(rend_depth[:,None,:,:], map_verts_proj[:,None,:,:])
        rend_depth_at_proj = rend_depth_at_proj.squeeze(1).squeeze(1)

        img_rgba = torch.cat([batch['img'], batch['mask'][:,None,:,:]], dim=1)
        img_rgba_at_proj = torch.nn.functional.grid_sample(img_rgba, map_verts_proj[:,None,:,:])
        img_rgba_at_proj = img_rgba_at_proj.squeeze(2)
...
class HMR2_4dhuman(PHALP):
    ...
    def track(self):
        ...
        while cap.isOpened():
            ...
            # Process frame here using PHALP
            tracked_frame = self.HMAR.forward(frame)

            # Add more processing or tracking logic if needed
